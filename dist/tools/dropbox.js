...
import { marshalAssignments, marshalAssignment, marshalSubmissions } from '../utils/marshal.js';
...
function getOrgUnitId(orgUnitId) {
...
function submissionCount(submissions) {
...
export const assignmentTools = {
    get_assignments: {
        ...
        handler: async ({ orgUnitId }) => {
            const folders = await client.getDropboxFolders(getOrgUnitId(orgUnitId));
    ...
    get_assignment: {
        ...
        handler: async ({ orgUnitId, assignmentId }) => {
            const folder = await client.getDropboxFolder(getOrgUnitId(orgUnitId), assignmentId);
    ...
    get_assignment_submissions: {
        ...
        handler: async ({ orgUnitId, assignmentId }) => {
            const submissions = await client.getDropboxSubmissions(getOrgUnitId(orgUnitId), assignmentId);
    ...
    submit_assignment: {
        ...
        handler: async ({ orgUnitId, assignmentId, filePath, comment = '', confirmed, allowResubmission = false, }) => {
            if (confirmed !== true) {
            ...
            const [assignment, upload] = await Promise.all([
                client.getDropboxFolder(courseId, assignmentId),
            ...
            if (assignment.SubmissionType !== undefined
            ...
            if (assignment.AllowableFileType === 5
                ...
                if (!allowed.includes(extension)) {
                    throw new Error(`File type ${extension || '(none)'} is not allowed; expected one of: ${allowed.join(', ')}`);
                }
            }
            const before = await client.getMyDropboxSubmissions(courseId, assignmentId);
            const previousSubmissions = submissionCount(before);
            const previousSubmissionIds = new Set(before.flatMap((entity) => entity.Submissions ?? []).map((submission) => submission.Id));
            if (previousSubmissions > 0 && !allowResubmission) {
                throw new Error(`Assignment already has ${previousSubmissions} submission(s); set allowResubmission=true only after explicit user approval`);
            }
            ...
            const after = await client.getMyDropboxSubmissions(courseId, assignmentId);
            const matchingSubmissions = after
                .flatMap((entity) => entity.Submissions ?? [])
                .filter((submission) => (!previousSubmissionIds.has(submission.Id)
                && submission.Files?.some((file) => file.FileName === upload.filename && file.Size === upload.size)))
