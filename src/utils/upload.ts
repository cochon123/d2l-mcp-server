...
export async function validateUpload(filePath: string): Promise<ValidatedUpload> {
  ...
  if (!roots.some((root) => isWithinRoot(resolvedPath, root))) {
    throw new Error(
      `Upload path is outside D2L_UPLOAD_ROOTS: ${resolvedPath}`
    );
  }
  ...
  if (fileStat.size > maxBytes) {
    throw new Error(
      `Upload is ${fileStat.size} bytes, exceeding D2L_MAX_UPLOAD_BYTES (${maxBytes})`
    );
  }
...
export function buildDropboxMultipartBody(
  upload: Pick<ValidatedUpload, 'filename' | 'contentType' | 'data'>,
  comment: string,
  boundary = `d2l-mcp-${randomBytes(18).toString('hex')}`
